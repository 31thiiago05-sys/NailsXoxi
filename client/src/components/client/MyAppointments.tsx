                            ...
                            <div key={apt.id} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-6 hover:shadow-md transition-shadow">
                                ...
                                <div className="w-full md:w-auto flex flex-col items-end gap-2">
                                    {apt.status === 'CONFIRMED' ? (
                                        <>
                                            <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium ${new Date(apt.date) < new Date() ? 'bg-gray-100 text-gray-700' : 'bg-green-100 text-green-700'
                                                }`}>
                                                <CheckCircle className="w-4 h-4" /> {new Date(apt.date) < new Date() ? 'Completado' : 'Confirmado'}
                                            </span>
